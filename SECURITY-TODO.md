...
- [ ] **H1 — No auth/authorization, exposed on all interfaces with wildcard CORS.**
      ...
      **Status:** CORS lever done — `CORS(app)` now reads `CORS_ALLOWED_ORIGINS` ([summa/**init**.py](summa/__init__.py#L49)) and defaults to deny (the PWA is same-origin), with `*` as an explicit opt-in. Network exposure (bind address) and request auth remain deployment concerns, documented in [README.md](README.md#L23); box stays unchecked until those are addressed for a given deployment.
